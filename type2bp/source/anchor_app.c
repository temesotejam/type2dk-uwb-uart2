/* Type2BP SR150 anchor: two independent sessions, with generated roles.
...
static bool ok(const char *step,tUWBAPI_STATUS s,uint32_t sid){
    PRINTF("BP_INIT,anchor=%04x,step=%s,sid=%08lx,status=%u\r\n",ANCHOR_ID,step,(unsigned long)sid,(unsigned)s);
...
static bool configure(unsigned i){
    ...
    r.multiNodeMode=s->multi;r.macAddrMode=0;
    ...
    const UWB_AppParams_List_t cfg[]={
        ...
        UWB_SET_APP_PARAM_VALUE(RESPONDER_SLOT_INDEX,s->slot),
    };
    ...
    if(check.deviceRole!=r.deviceRole||check.deviceType!=r.deviceType||check.multiNodeMode!=r.multiNodeMode||check.macAddrMode!=0||
    ...
    const struct {eAppConfig id;uint32_t value;} verify[]={
        {CHANNEL_NUMBER,s->channel},{SFD_ID,TAG_RADIO_SFD},{PREAMBLE_CODE_INDEX,TAG_RADIO_PREAMBLE},
        ...
        {RESPONDER_SLOT_INDEX,s->slot},
    };
    for(unsigned j=0;j<COUNT(verify);j++){
        uint32_t value=0; tUWBAPI_STATUS st=UwbApi_GetAppConfig(handles[i],verify[j].id,&value);
        if(st!=UWBAPI_STATUS_OK||value!=verify[j].value){PRINTF("BP_CONFIG_ERROR,anchor=%04x,param=%u,value=%lu,status=%u\r\n",ANCHOR_ID,(unsigned)verify[j].id,(unsigned long)value,(unsigned)st);return false;}
    }
    PRINTF("BP_SESSION,anchor=%04x,node=%u,sid=%08lx,handle=%08lx,peer=%04x,channel=%u,slot=%u\r\n",ANCHOR_ID,s->node,(unsigned long)s->id,(unsigned long)handles[i],s->peer,s->channel,s->slot);
    return true;
}
...
static bool recover(void){
    for(unsigned i=0;i<2;i++){
        ...
        if(state==UWBAPI_SESSION_IDLE){if(t-attempts[i]>5000u&&!start(i))return false;}
            ...
            uint32_t timeout=15000u+anchor_sessions[i].slot*211u+i*131u;
...
static OSAL_TASK_RETURN_TYPE run(void *p){
    ...
    PRINTF("BOOT,TYPE2BP_ANCHOR_V0.3.0,anchor=%04x,address=%04x\r\n",ANCHOR_ID,ANCHOR_ADDRESS);
    ...
    for(;;){
        ...
        for(unsigned i=0;i<2;i++)PRINTF("BP_STAT,anchor=%04x,node=%u,state=%u,reason=%u,ok=%lu,fail=%lu\r\n",ANCHOR_ID,anchor_sessions[i].node,states[i],reasons[i],(unsigned long)range_ok[i],(unsigned long)range_bad[i]);
        phOsalUwb_Delay(2000);
    }
fail:
    PRINTF("BP_RECOVER,anchor=%04x,action=REBOOT\r\n",ANCHOR_ID);phOsalUwb_Delay(3000+anchor_sessions[0].slot*211u);RESET_SystemReset();for(;;){}
}
