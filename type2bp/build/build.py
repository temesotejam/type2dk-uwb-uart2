...
subprocess.run([sys.executable,str(root.parent/'scripts/generate_profile.py'),a.profile,'--anchor',str(a.anchor),'--out',str(out/'anchor_profile.h')],check=True)
prefix=str(Path(a.gcc_bin).resolve()/'arm-none-eabi-') if a.gcc_bin else 'arm-none-eabi-'
p=sdk/'project/RhodesV4_SE'; cc=ET.parse(p/'.cproject').findall('.//cconfiguration')[0]
tool=next(t for t in cc.findall('.//tool') if t.get('name')=='MCU C Compiler')
opts={o.get('name'):o for o in tool.findall('option')}
defs=[v.get('value').replace(chr(92), '') for v in opts['Defined symbols (-D)']]
# CRP header and nohost glue belong to the IDE, not the hardware SDK.
defs=[v for v in defs if v not in ['__MCUXPRESSO','SERIAL_PORT_TYPE_SWO=1','UWBFTR_SE_SE051W=1']]
incs=[]
...
for l in ET.parse(p/'.project').findall('.//link'):
 ...
 for f in ([path] if path.is_file() else path.rglob('*')):
  ...
  sources[str(f)]=rel
if True:
 sources={f:r for f,r in sources.items() if 'demo_ranging_controller.c' not in f}
