...
static inline void range_view_update(range_cell_t cells[3],const event_t *e,uint64_t rx,bool reboot){
    if(reboot||e->type==EVENT_TEST)memset(cells,0,3*sizeof(*cells));
    if(e->anchor<1||e->anchor>3)return;
    range_cell_t *c=&cells[e->anchor-1];
